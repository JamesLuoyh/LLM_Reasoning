...
def _ensure_configurable(config: RunnableConfig) -> Configuration:
    ...
    return {
        **configurable,
        ...
        "n_voters": configurable.get("n_voters", 1),
...
def generate(state: State, *, config: RunnableConfig,
    ...
    for i in range(configurable["n_generators"]):
        ...
        if generation:
            ...
            reasonings.append("Not found.")
            solutions.append("Not found.")
...
def borda_count(state: State, debug: bool = False) -> Dict[str, Any]:
    ...
    n_generators = len(preferences[0])
    borda_counts = {i: 0 for i in range(len(n_generators))}
    for preference in preferences: 
