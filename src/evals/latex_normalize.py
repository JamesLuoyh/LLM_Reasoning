...
def _str_to_interval(x: str):
    try:
        ...
        for idx, or_interval in enumerate(x):
            ...
            for and_interval in or_interval.split(
                    "∩") if "∩" in or_interval else or_interval.split("\\cap"):
                and_interval = and_interval.strip()
                int_start, int_end = and_interval.split(",")
                int_start = latex2sympy_wrapper(
                    int_start.replace("(", "").replace("[", ""))
                int_end = latex2sympy_wrapper(
                    int_end.replace(
                        ")", "").replace(
                        "]", ""))
                # check validation
                ...
                temp_int = sympy.Interval(
                    int_start, int_end, left_open="(" in and_interval, right_open=")" in and_interval
                )
...
def _str_to_time_list(expr: str):
    ...
    if match:
        ...
        if meridiem and ("p.m." in meridiem.lower()
                         or "pm" in meridiem.lower()):
            hour = [
                str(int(hour) % 12),
                str(int(hour) % 12 + 12),
            ]
...
def _string_normalize(
    ...
    # latex matrix has \\\\ which cannot be replaced, e.g., \\begin{matrix} 1
    # & 2 \\\\ 3 & 4 \\end{matrix}
    expr = _str_matrix_normalize(expr)

    # Remove enclosing `\\text{}` or `\\mbox`
    m = re.search(
        r"^\$?\s*\\\s*(text|mbox|boxed){\s*(?P<text>.+?)}\s*\$?$",
        expr,
        re.DOTALL)
    if m:
        expr = m.group("text")
    ...
    if m:
        ...
        # sometimes the answer may not have "_base", we adopt the following
        # logic.
    ...
    # units in \\text{xxx}
    # Special case for time, transform to list, e.g., "08:30 \\text{pm}" ->
    # "[8, 30]"
...
def further_string_normalize(
    ...
    # In `expr_with_only_i` of latex_parser.py: "1+2i" => 1+2I, "i(2-i)" =>
    # I(2-I), so the two are equal.
    if lower_case:
        expr = expr.lower()
